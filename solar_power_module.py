...
# - Submit + Refresh
# - Shows last 10 entries (Filtered by Toll Plaza)
...
def run():
    ...
    # --- Recent entries (Filtered by Toll Plaza) ---
    st.markdown("---")
    st.caption(f"Last 10 entries for {toll_plaza}")

    try:
        rows = (
            client.table("solar_generation")
            .select("*")
            .eq("toll_plaza", toll_plaza)  # ✅ Filter by selected plaza
            .order("id", desc=True)
            .limit(10)
            .execute()
        )
        ...
        if getattr(rows, "data", None):
            ...
            st.info(f"No records found for {toll_plaza}.")
