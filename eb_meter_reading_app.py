# eb_meter_reading_app.py — import-safe, with post-submit rerun (no session_state writes to widget keys)
import os
...
def f2(text: str, name: str):
    text = (text or "").strip()
    if text == "": return 0.0, None
    try: return float(text), None
    except Exception: return None, f"❌ {name}: enter a number."
 
def fetch_open(db, plaza: str):
    r = (db.table("eb_live_status")
...
def upsert_live(db, plaza: str, consumer: str, okwh: float, okvah: float):
    return (db.table("eb_live_status")
              .upsert({"toll_plaza": plaza, "consumer_number": consumer,
                       "opening_kwh": okwh, "opening_kvah": okvah})
              .execute())
...
def run():
    ...
    if page == "User Entry":
        ...
        with c2: plaza = st.selectbox("Toll Plaza", PLAZAS)
            ...
            with st.expander("🧰 Quick Init/Update Opening"):
                ...
                if st.button("Save Opening", key=f"save_open_{plaza}"):
                    ...
                    if not (e1 or e2):
                        ...
                        if getattr(res,"error",None): st.error(f"❌ {res.error}")
        ...
        if k_ckvh not in st.session_state:  st.session_state[k_ckvh]  = f"{okvah:.2f}"
        ...
        if sub:
            ...
            payload = {
                ...
                "opening_kvah": okvah, "closing_kvah": ckvh, "net_kvah": net_kvah,
            ...
            # DO NOT assign to widget keys here; just clear context and rerun
